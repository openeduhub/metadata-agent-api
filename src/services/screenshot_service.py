...
class ScreenshotService:
    ...
    async def capture(
        ...
        if method == "playwright":
            return await self._capture_playwright(url, width, height, format, full_page, delay)
        else:
            return await self._capture_pageshot(url, width, height, format, full_page, delay)
    ...
    async def _capture_playwright(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                capture_time = int((time.monotonic() - start) * 1000)
                logger.info(f"✅ Playwright: {len(image_bytes)} bytes ({capture_time}ms)")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=45.0) as client:
                response = await client.post(
                    ...
                    files={"image": ("screenshot.png", result.image_bytes, result.mimetype)},
                )
                ...
                if response.status_code in (200, 201, 204):
                    ...
                    logger.error(f"❌ Preview upload failed: {response.status_code} {error}")
                    return {
                        "success": False,
